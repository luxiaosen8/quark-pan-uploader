...
class UploadWorker(QObject):
    ...
    def _build_executor(self):
        return call_with_supported_kwargs(self.executor_factory, logger_callback=self.log_message.emit)
    ...
    def run(self) -> None:
            ...
            for index, job in enumerate(self.plan.jobs):
                if self.cancel_token.is_cancelled():
                    for pending_job in self.plan.jobs[index:]:
                        self.task_status.emit(pending_job.local_name, FolderTaskStatus.STOPPED.value, '', 0)
                    final_state = 'stopped'
                    break
                self.current_action.emit(f'当前任务：{job.local_name}')
                self.task_status.emit(job.local_name, FolderTaskStatus.UPLOADING.value, '', 0)
                status_callback = lambda status, share_url="", retry_count=0, local_name=job.local_name: self.task_status.emit(
                    local_name,
                    status,
                    share_url,
                    retry_count,
                ...
                try:
                    ...
                    failed += 1
                    self.task_status.emit(job.local_name, FolderTaskStatus.FAILED.value, '', 0)
                    self.log_message.emit(f'[ERROR] 后台任务失败：{job.local_name} -> {exc}')
                    self.progress_summary.emit(completed, total, failed)
                    final_state = 'completed_with_errors'
                    continue
                self.task_status.emit(job.local_name, getattr(result, 'status', FolderTaskStatus.COMPLETED.value), getattr(result, 'share_url', ''), getattr(result, 'retry_count', 0))
                state = getattr(result, 'status', FolderTaskStatus.COMPLETED.value)
        ...
        self.current_action.emit('当前动作：空闲' if final_state in {'completed', 'completed_with_errors'} else ('当前动作：已停止' if final_state == 'stopped' else '当前动作：失败'))
