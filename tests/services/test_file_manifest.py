...
from quark_uploader.services.file_manifest import LocalFileEntry, build_folder_file_manifest, build_task_file_manifest
...
def test_build_task_file_manifest_supports_single_file_task(tmp_path: Path):
    file_path = tmp_path / "cover.txt"
    file_path.write_text("123", encoding="utf-8")

    entries = build_task_file_manifest(
        FolderTask(local_name="cover.txt", local_path=str(file_path), file_count=1, total_size=3, source_type=TaskSourceType.FILE)
    )

    assert [entry.relative_path for entry in entries] == ["cover.txt"]
